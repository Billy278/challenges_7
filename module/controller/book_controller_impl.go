...
func (book_ctr *BookControllerImpl) UpdateBookCtr(ctx *gin.Context) {
	...
	reqIn.Id = id
	book, err := book_ctr.BookSrv.UpdateBookSrv(ctx, reqIn)
	if err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, response.ResWeb{
			Status: err.Error(),
		})
...
func (book_ctr *BookControllerImpl) DeleteBookCtr(ctx *gin.Context) {
	id, err := book_ctr.getIdFromParam(ctx)
	if err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, response.ResWeb{
			Code:   http.StatusBadRequest,
	...
	book, err := book_ctr.BookSrv.DeleteBookSrv(ctx, id)
	if err != nil {
		ctx.AbortWithStatusJSON(http.StatusBadRequest, response.ResWeb{
			Code:   http.StatusBadRequest,
			Status: err.Error(),
	...
	ctx.JSON(http.StatusOK, response.ResWeb{
		Code:   http.StatusOK,
		Status: "success delele book with id",
		Data:   book,
	})
